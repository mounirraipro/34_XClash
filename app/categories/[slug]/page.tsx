...
export async function generateMetadata(
    ...
    if (!cat) {
        return {
            ...
            description: 'The requested category could not be found on X-Clash.',
            robots: {
                index: false,
                follow: false,
            },
    ...
    const description = `Play ${cat.name.toLowerCase()} puzzles on X-Clash. ${cat.description} ${cat.levels.length} levels from easy to expert.`;
    const canonicalUrl = `/categories/${cat.slug}`;

    return {
        title,
        description,
        keywords: [`${cat.name.toLowerCase()} puzzles`, `${cat.name.toLowerCase()} jigsaw`, `X-Clash ${cat.name.toLowerCase()}`, 'free puzzle game'],
        alternates: {
